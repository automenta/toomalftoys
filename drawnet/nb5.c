  ...
  for (z = 0;z<nn;z++) {
    for (y = 2;y<ns;y++) {
      //if ((df[brain[z][ (y) ]]>0)&&(brain[z][0]<= 4000000000 - df[brain[z][ (y) ]] )) {
           brain[z][0]+= df[brain[z][(y)]]*10; // 1-weight excitatory synapses only   
    //  }kg
    //  if ((df[z]>0)&&(brain[brain[z][ (y) ]][0]<= 4000000000 -df[z] )) {
          brain[brain[z][ (y) ]][0]+= df[z] *5 ; // synapses are bidirectional   
//      }
