...
float cosdiv= (((float)(rand()%1000))+1)/100.0;
  ...
  for (x=0;x< cols;x++){
   ...
   if (ok==0)   {
      ...
      ok=2;
              brain[spike%nn][0]+= brain[spike%nn][1]+ event.motion.y* 1000000;// 4100000000 ;
     }
   ...
   ok=0;
    ...
    oldspike=spike;
             spike= event.motion.x/zoom%nn;
              brain[spike%nn][0]+= brain[spike%nn][1]+event.motion.y*1000000;
                }
...
df[z] = ((brain[z][1]/10000))+( 100* (brain[z][0]/brain[z][1] ));
       brain[z][0] = brain[z][0]%brain[z][1];

  //if (brain[z][1]<= 4000000000 -(df[z])) {
 //  brain[z][1]*=tan(brain[z][1])+.999;//(pow(1.00044,((float)df[z])));
  brain[z][1]+=df[z]*50000;
  for(x=0;x<df[z];x++){brain[z][(rand()%((ns-2)))+2]=(((Uint8) (((brain[z][1]/1000000) ) %nn) ));}
   // } else { brain[z][1]= 4000000000 ; }
     } else { df[z]=0;
       //if ( (brain[z][0]==0) && (brain[z][1]>=10+1)) {
    if ((brain[z][1]>=4)) {
     brain[z][1]*= 0.92999;
   for(x=0;x<2;x++){ brain[z][(rand()%((ns-2)))+2]=((brain[z][1]/1000000))%nn;}
   //
  ...
  for (z = 0;z<nn;z++) {
     if (brain[z][0]>0) {
           ...
           brain[z][0]*=0.99;//03; // no potential to k
      }
    }
